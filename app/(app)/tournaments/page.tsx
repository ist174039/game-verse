...
import { ArrowRight, CalendarDays, CupSoda, LockKeyhole, Swords, Trophy } from 'lucide-react'
...
export default async function CompetitionsPage({ searchParams }: { searchParams: Promise<{ universe?: string }> }) {
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-2xl border border-white/[0.07] bg-[#0b0b0b] p-5 sm:p-6">
      ...
      <div className="mt-5 divide-y divide-white/[0.06] border-y border-white/[0.06]">{hub.competitions.length===0?<p className="py-10 text-center text-sm text-muted-foreground">Não existem competições ativas ou configuradas neste universo.</p>:hub.competitions.map(competition=><Link href={`/tournaments/${competition.id}`} key={competition.id} className="group grid gap-3 py-4 transition hover:bg-white/[0.015] sm:grid-cols-[1fr_auto_auto_auto] sm:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-bold group-hover:text-primary">{competition.name}</p><span className="rounded-md border border-primary/12 bg-primary/[0.035] px-2 py-1 text-[9px] font-bold uppercase tracking-[0.13em] text-primary">{competition.type}</span></div><p className="mt-1 text-xs text-muted-foreground">{competition.seasonId?'Associada a época':'Sem época associada'} · criada {new Date(competition.createdAt).toLocaleDateString('pt-PT')}</p></div><span className="w-fit rounded-md border border-white/[0.07] px-2 py-1 text-[9px] font-bold uppercase tracking-[0.13em] text-muted-foreground">{competition.status}</span><div className="text-left sm:text-right"><p className="text-sm font-black tabular-nums text-[var(--silver)]">{competition.prizePool.toLocaleString('pt-PT')} S</p><p className="text-[9px] uppercase tracking-[0.12em] text-muted-foreground">prize pool</p></div><ArrowRight className="h-4 w-4 text-white/20 transition group-hover:translate-x-0.5 group-hover:text-primary"/></Link>)}</div>
      <p className="mt-4 text-[11px] leading-5 text-muted-foreground">O detalhe abre diretamente competition, participants, rounds, standings e fixtures. O schema tournament legado deixou de participar neste fluxo.</p>
    </section>
  </div>
}
