...
export default async function AuctionPage({ searchParams }: { searchParams: Promise<{ universe?: string }> }) {
  ...
  return <div className="space-y-7">
    ...
    <section className="brand-watermark rounded-2xl border border-white/[0.07] bg-[#0b0b0b] px-5 py-6 sm:px-7"><div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><p className="clan-kicker">Leilões · {market.universe.name}</p><h1 className="mt-2 text-3xl font-black tracking-[-0.04em] sm:text-4xl">Escrow real. Concorrência transparente.</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Cada licitação passa pelo ledger do universo. O maior bid mantém Silver reservado; quando é superado, o escrow anterior é libertado pela operação transacional.</p></div><CurrencyDisplay kind="silver" amount={market.silverBalance} label="Silver disponível" /></div></section>
