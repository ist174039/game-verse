...
export default async function PlayPage({ searchParams }: { searchParams: Promise<{ universe?: string }> }) {
  ...
  if (!hub) return <GuestCompetition signedIn />

  return (
    <div className="space-y-7">
      <CompetitionHeader universeName={hub.universe.name} />
      <section className="grid gap-px overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.07] sm:grid-cols-3">
        <StatusDatum icon={Trophy} label="Elo do clube" value={hub.club.elo.toLocaleString('pt-PT')} detail="Específico deste universo" />
        <StatusDatum icon={Gamepad2} label="Partidas" value={(hub.activeMatches.length + hub.completedMatches.length).toString()} detail={`${hub.activeMatches.length} ainda não liquidadas`} />
        <StatusDatum icon={ShieldCheck} label="Lifecycle final" value="SETTLED" detail="Só este estado produz consequências finais" accent />
      </section>
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <CreateMatch universeName={hub.universe.name} eloRating={hub.club.elo} silverBalance={hub.silverBalance} competitionCount={hub.competitions.length} />
        <div className="space-y-6">
          <MatchList matches={hub.activeMatches} universeId={hub.universe.id} title="Partidas em curso" emptyMessage="Não existem partidas pendentes neste universo." icon="active" />
          <MatchList matches={hub.completedMatches} universeId={hub.universe.id} title="Histórico liquidado" emptyMessage="Ainda não existem partidas liquidadas." icon="history" />
        </div>
      </div>
    </div>
  )
