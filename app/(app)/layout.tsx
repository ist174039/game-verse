...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [profileResult,currenciesResult,clubsResult,silverResult,universesResult,adminResult] = !isGuest
    ? await Promise.all([
        ...
        supabase.from('club').select('id,name,universe_id,logo_url').eq('user_id',user.id),
  ...
  const universeContexts = ((clubsResult.data ?? []) as Array<{id:string;name:string;universe_id:string;logo_url:string|null}>)
    .map(club=>{const universe=universes.get(club.universe_id);return universe?{id:universe.id,name:universe.name,kind:universe.kind,clubName:club.name,clubLogoUrl:club.logo_url,silverBalance:silverByClub.get(club.id)??0}:null})
