...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  if (!hasSupabasePublicConfig()) {
    return <BackendConfigurationRequired />
  }
...
function BackendConfigurationRequired() {
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#050505] px-4 py-10">
      <section className="w-full max-w-3xl rounded-2xl border border-primary/15 bg-[#0b0b0b] p-6 sm:p-8">
        <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-primary/15 bg-primary/[0.05] text-primary">
          <Database className="h-5 w-5" />
        </div>
        <div className="mt-5 flex items-center gap-2 text-primary">
          <ShieldAlert className="h-4 w-4" />
          <p className="text-xs font-bold uppercase tracking-[0.16em]">Backend não configurado</p>
        </div>
        <h1 className="mt-3 text-2xl font-black tracking-[-0.03em] sm:text-3xl">O Clã das Sombras precisa do Supabase de produção.</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">A aplicação foi publicada sem as variáveis públicas do projeto Supabase. As áreas autenticadas e o Admin ficam indisponíveis até o backend definitivo ser ligado ao ambiente Production da Vercel.</p>

        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <ConfigItem name="NEXT_PUBLIC_SUPABASE_URL" />
          <ConfigItem name="NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY" />
        </div>
...
function ConfigItem({ name }: { name: string }) {
  return <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-3"><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-muted-foreground">Variável necessária</p><code className="mt-1 block break-all text-xs font-semibold text-foreground">{name}</code></div>
}
