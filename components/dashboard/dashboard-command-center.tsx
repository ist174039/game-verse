...
export function DashboardCommandCenter({dashboard}:{dashboard:DashboardReadModel}){
  ...
  const competitionLinked=o.registeredCompetitions>0||o.activeCompetitions>0
  const next=dashboard.nextMatchContext
  const waitingForCompetition=o.registeredCompetitions>0&&o.activeCompetitions===0&&!next
  ...
  else if(!competitionLinked&&o.availableCompetitions>0){primary={title:'Escolhe a primeira competição',description:`Existem ${o.availableCompetitions} competição(ões) abertas a inscrições neste universo.`,href:`/tournaments?universe=${dashboard.universe.id}`,label:'Ver competições',icon:Swords,tone:'default'}}
  ...
  else if(waitingForCompetition){primary={title:'Inscrição concluída · aguarda calendário',description:'O teu clube já está inscrito. A plataforma precisa de ativar a competição e gerar o calendário quando existirem participantes suficientes.',href:`/tournaments?universe=${dashboard.universe.id}`,label:'Ver inscrição',icon:CalendarClock,tone:'default'}}
  ...
  const competitionValue=waitingForCompetition?'Inscrito · aguarda calendário':competitionLinked?`${o.registeredCompetitions+o.activeCompetitions} vínculo(s)`:`${o.availableCompetitions} disponível(eis)`
  return <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
    ...
    <article className="rounded-2xl border border-white/[0.07] bg-[#0b0b0b] p-5 sm:p-6"><div className="flex items-center gap-2"><Shield className="h-4 w-4 text-primary"/><h2 className="text-lg font-black">Estado do clube</h2></div><div className="mt-5 space-y-3"><Step label="Clube criado" value={dashboard.club.name} ready/><Step label="Plantel competitivo" value={`${o.squadSize}/${dashboard.universe.minSquadSize} jogadores`} ready={squadReady}/><Step label="Infraestrutura base" value={`${o.infrastructureCount}/5 módulos`} ready={infraReady}/><Step label="Competição" value={competitionValue} ready={competitionLinked}/><Step label="Primeira partida" value={next?`${next.homeClubName} vs ${next.awayClubName}`:'Ainda não agendada'} ready={Boolean(next)}/></div></article>
  </section>
}
