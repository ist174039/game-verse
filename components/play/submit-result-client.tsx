...
export function SubmitResultClient({ matchId, userId, universeId, isHome, homeClubName, awayClubName }: { matchId: string; userId: string; universeId: string; isHome: boolean; homeClubName: string; awayClubName: string }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const home = Number(homeScore); const away = Number(awayScore)
    if (!Number.isSafeInteger(home) || !Number.isSafeInteger(away) || home < 0 || away < 0) { setError('Introduz resultados inteiros e não negativos.'); return }
    if (evidence && evidence.size > 25 * 1024 * 1024) { setError('A evidência não pode exceder 25 MB.'); return }

    setLoading(true)
    try {
      if (evidence) {
        const ext = evidence.name.split('.').pop()?.toLowerCase() || 'bin'
        const path = `${matchId}/${userId}-${crypto.randomUUID()}.${ext}`
        const supabase = createClient()
        const { error: uploadError } = await supabase.storage.from('match-evidence').upload(path, evidence, { upsert: false })
        if (uploadError) throw new Error(`evidence_upload_failed: ${uploadError.message}`)
      }

      const response = await fetch('/api/competition/submit-result', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ matchId, homeScore: home, awayScore: away, idempotencyKey: `match-submit:${crypto.randomUUID()}` }) })
      const payload = await response.json()
      if (!response.ok) throw new Error(payload.error || 'submit_result_failed')
      router.push(`/play?universe=${universeId}`); router.refresh()
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Não foi possível submeter o resultado.') }
...
function ScoreField({ id, label, value, onChange }: { id: string; label: string; value: string; onChange: (value: string) => void }) { return <div className="space-y-2"><Label htmlFor={id} className="line-clamp-1">{label}</Label><Input id={id} inputMode="numeric" value={value} onChange={event => onChange(event.target.value.replace(/[^0-9]/g, ''))} placeholder="0" className="h-16 text-center text-2xl font-black" required /></div> }
