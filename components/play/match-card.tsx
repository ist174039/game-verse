...
import { AlertTriangle, CalendarClock, CheckCircle2, Clock, FilePenLine, Shield, Swords } from 'lucide-react'
...
export function MatchCard({ context, universeId }: { context: MatchContextReadModel; universeId: string }) {
  ...
  const cancelled = match.state === 'CANCELLED'
  const disputed = match.state === 'DISPUTED'
  const hasScore = match.homeScore != null && match.awayScore != null
  const deadlineValue = match.state === 'RESULT_SUBMITTED' ? match.resultMetadata?.confirmation_deadline : null
  const confirmationDeadline = typeof deadlineValue === 'string' ? deadlineValue : null
  ...
  return <>
    <article className="rounded-xl border border-white/[0.07] bg-[#0b0b0b] p-4 transition hover:border-primary/18">
      ...
      {confirmationDeadline && <div className="mt-4 rounded-lg border border-primary/12 bg-primary/[0.035] px-3 py-2 text-[11px] leading-5 text-muted-foreground"><Clock className="mr-1.5 inline h-3.5 w-3.5 text-primary" />{context.canConfirm ? 'Confirma ou abre disputa até ' : 'O adversário pode confirmar ou disputar até '}<strong className="text-foreground">{new Date(confirmationDeadline).toLocaleString('pt-PT')}</strong>. Depois do prazo, o sistema pode auto-confirmar se não existir disputa aberta.</div>}
      {error && <p className="mt-4 rounded-lg border border-red-500/20 bg-red-500/[0.05] p-3 text-xs text-red-300">{error}</p>}
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-white/[0.06] pt-3"><p className="text-[10px] text-muted-foreground">{match.scheduledAt ? new Date(match.scheduledAt).toLocaleString('pt-PT') : new Date(match.createdAt).toLocaleString('pt-PT')}</p><div className="flex flex-wrap gap-2">{!final&&!cancelled&&<Button size="sm" variant="ghost" asChild><Link href={`/play/${match.id}/pre-match`}><CalendarClock className="mr-1.5 h-3.5 w-3.5" />Pré-jogo</Link></Button>}{context.canSubmit && <Button size="sm" variant="outline" asChild><Link href={`/play/${match.id}/submit-result?universe=${universeId}`}><FilePenLine className="mr-1.5 h-3.5 w-3.5" />Submeter resultado</Link></Button>}{context.canConfirm && <Button size="sm" onClick={() => setConfirmOpen(true)}>Confirmar</Button>}{context.canDispute && !disputed && !final && <Button size="sm" variant="outline" onClick={() => setDisputeOpen(true)}>Disputar</Button>}</div></div>
    </article>
