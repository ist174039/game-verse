...
import type { PlayerAssetStatus } from '@/lib/domain/core'
  ...
  return (
    <article className={cn('group relative overflow-hidden rounded-2xl border border-white/[0.07] bg-[#0b0b0b]', className)}>
      <div className={cn('relative overflow-hidden bg-[radial-gradient(circle_at_50%_22%,rgba(245,191,22,.14),transparent_38%),linear-gradient(180deg,#15130c,#080808)]', compact ? 'aspect-[5/3]' : 'aspect-[4/5]')}>
        ...
        <div className="absolute left-4 top-4"><p className="text-3xl font-black leading-none tracking-[-0.05em] text-primary">{player.rating}</p><p className="mt-1 text-[9px] font-black uppercase tracking-[0.16em] text-white/60">{player.position}</p></div>
      ...
      <div className={cn('relative', compact ? 'p-4' : '-mt-12 p-5 pt-0')}>
        ...
        {player.nationality && <p className="mt-1 truncate text-[11px] text-muted-foreground">{player.nationality}</p>}
        <div className="mt-4 grid grid-cols-2 gap-3 border-t border-white/[0.06] pt-4">
          <PlayerDatum icon={Coins} label="Valor ref." value={player.marketValue == null ? '—' : `${player.marketValue.toLocaleString('pt-PT')} S`} />
          <PlayerDatum icon={Activity} label={player.salaryIsReference ? 'Salário ref.' : 'Salário'} value={player.salary == null ? '—' : `${player.salary.toLocaleString('pt-PT')} S`} />
        </div>
      </div>
    </article>
  )
