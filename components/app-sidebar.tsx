...
interface SidebarUniverseContext { id:string; name:string; kind:string; clubName:string; silverBalance:number }
interface AppSidebarProps { username?: string; currencyBalances?:{gold:number;bronze:number}; universeContexts?:SidebarUniverseContext[]; isGuest?: boolean; adminRole?: string | null }
export function AppSidebar({ username='Manager', currencyBalances={gold:0,bronze:0}, universeContexts=[], isGuest=false, adminRole=null }: AppSidebarProps) {
  const hasInternalAccess=Boolean(adminRole)
  const pathname=usePathname();const searchParams=useSearchParams();const router=useRouter();const[mobileOpen,setMobileOpen]=useState(false);const[logoutOpen,setLogoutOpen]=useState(false);const[isLoggingOut,setIsLoggingOut]=useState(false)
  const requestedUniverseId=searchParams.get('universe')
  const activeUniverse=universeContexts.find(context=>context.id===requestedUniverseId)??universeContexts[0]??null
  const changeUniverse=(universeId:string)=>{const params=new URLSearchParams(searchParams.toString());params.set('universe',universeId);const safePath=NAVIGATION_ROOTS.has(pathname)||pathname.startsWith('/admin')?pathname:'/dashboard';router.replace(`${safePath}?${params.toString()}`)}
  ...
  const NavContent=()=> <>
    ...
    {!isGuest&&<SessionContextCard activeUniverse={activeUniverse} universeContexts={universeContexts} currencyBalances={currencyBalances} onUniverseChange={changeUniverse}/>}
    <nav className="flex-1 space-y-1 overflow-y-auto pr-1"><NavGroup title="Clube" items={mainNavItems} pathname={pathname} universeId={activeUniverse?.id} close={()=>setMobileOpen(false)}/><NavGroup title="Plataforma" items={secondaryNavItems} pathname={pathname} universeId={activeUniverse?.id} close={()=>setMobileOpen(false)}/>{hasInternalAccess&&<><div className="my-3 h-px bg-white/[.05]"/><div className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[.15em] text-muted-foreground/60">Operações restritas</div>{adminNavItems.map(item=><NavLink key={item.href} item={item} pathname={pathname} universeId={activeUniverse?.id} close={()=>setMobileOpen(false)} destructive/>)}</>}</nav>
    <div className="mt-4 border-t border-white/[.055] pt-4"><Link href="/profile" onClick={()=>setMobileOpen(false)} className="mb-2 flex items-center gap-3 rounded-xl px-2 py-2 transition-colors hover:bg-white/[.035]"><div className="flex h-9 w-9 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-sm font-bold text-primary">{username.charAt(0).toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{username}</p><p className="truncate text-[10px] uppercase tracking-wider text-muted-foreground">{adminRole?adminRole.replaceAll('_',' '):'Manager'}</p></div></Link><Button variant="ghost" className="w-full justify-start text-muted-foreground hover:bg-destructive/10 hover:text-destructive" onClick={()=>setLogoutOpen(true)}><LogOut className="h-[18px] w-[18px]"/> Sair</Button></div>
  </>
  return <>
...
function SessionContextCard({activeUniverse,universeContexts,currencyBalances,onUniverseChange}:{activeUniverse:SidebarUniverseContext|null;universeContexts:SidebarUniverseContext[];currencyBalances:{gold:number;bronze:number};onUniverseChange:(universeId:string)=>void}){return <section className="mb-5 overflow-hidden rounded-xl border border-primary/10 bg-primary/[.025]"><div className="border-b border-white/[.06] p-3"><div className="flex items-center justify-between gap-3"><span className="inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[.14em] text-muted-foreground"><Globe className="h-3.5 w-3.5 text-primary"/>Universo ativo</span>{activeUniverse&&<span className="rounded-md border border-primary/12 bg-primary/[.05] px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[.12em] text-primary">{activeUniverse.kind==='MAIN'?'Oficial':'Comunidade'}</span>}</div>{universeContexts.length>1?<select aria-label="Universo ativo" value={activeUniverse?.id??''} onChange={event=>onUniverseChange(event.target.value)} className="mt-2 h-9 w-full rounded-lg border border-white/[.08] bg-black/30 px-2.5 text-xs font-bold text-foreground outline-none transition focus:border-primary/35">{universeContexts.map(context=><option key={context.id} value={context.id}>{context.name}</option>)}</select>:<p className="mt-2 truncate text-sm font-bold text-foreground">{activeUniverse?.name??'Sem universo ativo'}</p>}<p className="mt-1 truncate text-[10px] text-muted-foreground">{activeUniverse?activeUniverse.clubName:'Cria um clube para ativar Silver.'}</p></div><div className="p-3"><div className="mb-2 flex items-center justify-between"><span className="inline-flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[.14em] text-muted-foreground"><Coins className="h-3.5 w-3.5 text-primary"/>Carteira</span><Link href={contextualHref('/economy',activeUniverse?.id)} className="text-[9px] font-semibold text-primary transition hover:text-primary/80">Detalhes</Link></div><CurrencyRow label="Gold" value={currencyBalances.gold} tone="gold"/><CurrencyRow label="Silver" value={activeUniverse?.silverBalance??null} tone="silver"/><CurrencyRow label="Bronze" value={currencyBalances.bronze} tone="bronze"/></div></section>}
function CurrencyRow({label,value,tone}:{label:string;value:number|null;tone:'gold'|'silver'|'bronze'}){return <div className="flex items-center justify-between gap-3 border-t border-white/[.045] py-1.5 first:border-t-0"><span className="inline-flex items-center gap-2 text-[10px] text-muted-foreground"><span className={cn('h-2 w-2 rounded-full',tone==='gold'?'bg-primary':tone==='silver'?'bg-[#b9c0ca]':'bg-[#b86b3d]')}/>{label}</span><span title={value===null?undefined:value.toLocaleString('pt-PT')} className="max-w-[8.5rem] truncate text-right text-xs font-black tabular-nums text-foreground">{value===null?'—':value.toLocaleString('pt-PT')}</span></div>}
const NAVIGATION_ROOTS=new Set([...mainNavItems,...secondaryNavItems].map(item=>item.href))
function contextualHref(href:string,universeId?:string){if(!universeId)return href;const separator=href.includes('?')?'&':'?';return `${href}${separator}universe=${encodeURIComponent(universeId)}`}
function NavGroup({title,items,pathname,universeId,close}:{title:string;items:NavItem[];pathname:string;universeId?:string;close:()=>void}){return <div className="mb-4"><div className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[.15em] text-muted-foreground/60">{title}</div>{items.map(item=><NavLink key={item.href} item={item} pathname={pathname} universeId={universeId} close={close}/>)}</div>}
function NavLink({item,pathname,universeId,close,destructive=false}:{item:NavItem;pathname:string;universeId?:string;close:()=>void;destructive?:boolean}){const active=pathname===item.href||pathname.startsWith(item.href+'/');return <Link href={contextualHref(item.href,universeId)} onClick={close} className={cn('group flex min-h-10 items-center gap-3 rounded-xl px-3 py-2 text-[13px] font-medium transition-all',active?(destructive?'bg-destructive/10 text-destructive':'bg-primary/[.09] text-primary shadow-[inset_2px_0_0_#f5bf16]'):'text-[#aaa69e] hover:bg-white/[.035] hover:text-foreground')}><span className={cn('transition-colors',active&&!destructive?'text-primary':'text-[#77736c] group-hover:text-foreground')}>{item.icon}</span><span>{item.label}</span>{item.badge&&<span className="ml-auto rounded bg-white/[.05] px-1.5 py-.5 text-[9px]">{item.badge}</span>}</Link>}
function MobileNavLink({item,pathname,universeId}:{item:NavItem;pathname:string;universeId?:string}){const active=pathname===item.href||pathname.startsWith(item.href+'/');return <Link href={contextualHref(item.href,universeId)} className={cn('flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl px-1 text-[10px] font-semibold transition-colors',active?'bg-primary/[.09] text-primary':'text-muted-foreground hover:bg-white/[.035] hover:text-foreground')}>{item.icon}<span className="max-w-full truncate">{item.label}</span></Link>}
