...
export class SupabaseInternalJobsRepository implements InternalJobsRepository{
  ...
  async runMaintenance(input:{eventLimit?:number;listingLimit?:number;matchLimit?:number;auctionLimit?:number;competitionLimit?:number;jobKey:string}):Promise<InternalJobResult>{
    ...
    const[events,listings,loans,autoStart,ready,progress,auctions,sponsors,cycles]=await Promise.all([
    ...
    for(const result of[events,listings,loans,autoStart,ready,progress,auctions,sponsors,cycles]){
