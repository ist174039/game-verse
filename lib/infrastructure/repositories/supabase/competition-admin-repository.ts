...
import type { CompetitionAdminRepository, MatchDisputeDecision } from '@/lib/application/admin-contracts'
...
export class SupabaseCompetitionAdminRepository implements CompetitionAdminRepository {
  ...
  async activate(input:{competitionId:string;actorUserId:string;startsAt?:string|null;roundIntervalDays?:number}):Promise<Record<string,unknown>>{
    const {data,error}=await this.client.rpc('service_activate_competition',{
      p_competition_id:input.competitionId,
      p_actor_user_id:input.actorUserId,
      p_starts_at:input.startsAt??new Date().toISOString(),
      p_round_interval_days:input.roundIntervalDays??7,
    })
  ...
  async progress(input:{competitionId:string;actorUserId:string}):Promise<Record<string,unknown>>{
    const {data,error}=await this.client.rpc('service_progress_competition',{
      p_competition_id:input.competitionId,
      p_actor_user_id:input.actorUserId,
    })
    if(error)throw error
  ...
  async resolveDispute(input:{
    ...
    const {data,error}=await this.client.rpc('service_resolve_match_dispute',{
      p_dispute_id:input.disputeId,
      p_decision:input.decision,
      p_resolution:input.resolution,
      p_actor_user_id:input.actorUserId,
      p_idempotency_key:input.idempotencyKey,
      p_home_score:input.homeScore??null,
      p_away_score:input.awayScore??null,
    })
    if(error)throw error
    return data??{}
  }
