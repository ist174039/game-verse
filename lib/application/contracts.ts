...
export interface CompetitionRepository {
  ...
  listParticipants(competitionId: UUID): Promise<CompetitionParticipant[]>
  listStandings(competitionId: UUID): Promise<LeagueStanding[]>
  listClubMatches(clubId: UUID): Promise<Match[]>
  submitResult(input: { matchId: UUID; homeScore: number; awayScore: number; idempotencyKey: string }): Promise<Match>
  confirmResult(input: { matchId: UUID; idempotencyKey: string }): Promise<MatchSettlementReceipt>
  openDispute(input: { matchId: UUID; reason: string }): Promise<MatchDispute>
}
