-- Clã das Sombras — market guard runtime safety
-- Apply after 00420_market_competitive_roster_guards.sql.
--
-- This migration hardens DELETE-trigger handling and preserves the player's pre-listing
-- operational state when an auction expires without bids.
...
begin
  ...
  if tg_op='DELETE' then
    ...
    v_releasing:=new.status is distinct from 'ACTIVE';
  end if;

  if not v_releasing then
    return case when tg_op='DELETE' then old else new end;
  end if;
  ...
  loop
    begin
      ...
      if not found then
        ...
        set status=case when owner_club_id is null then 'AVAILABLE' else v_restore end,updated_at=now()
