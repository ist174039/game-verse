-- Clã das Sombras — automatic competition start and immediate match readiness
-- Apply after 00370_match_settlement_runtime.sql.
--
-- Official/bootstrap competitions may opt into automatic activation through rules.auto_start.
-- Activation remains service-role only. Admin-triggered activations keep actor_user_id for audit;
-- scheduled/system activations use actor_user_id = NULL and are explicitly marked automatic.

update public.competition c
set rules = coalesce(c.rules,'{}'::jsonb) || jsonb_build_object(
  'auto_start', true,
  'min_participants', 2,
  'round_interval_days', coalesce(nullif(c.rules->>'round_interval_days','')::integer,7)
)
from public.universe u
where c.universe_id=u.id
  and u.slug='principal'
  and c.name='Liga Principal'
...
begin
  ...
  if v_comp.type='LEAGUE' then
    ...
    for v_round in 1..(v_n-1) loop
      ...
      v_match_state:=case when v_scheduled_at<=now() then 'READY' else 'SCHEDULED' end;
...
create or replace function public.service_auto_start_competitions(p_limit integer default 50)
returns integer
language plpgsql
security definer
set search_path=public
...
declare
  ...
  v_started integer:=0;
begin
  if p_limit<1 or p_limit>500 then raise exception 'invalid_processing_limit'; end if;

  for v_comp in
    select c.id,c.rules,c.created_at
    from public.competition c
    where c.status in ('REGISTRATION','OPEN','SCHEDULED')
      and coalesce((c.rules->>'auto_start')::boolean,false)=true
    order by c.created_at,c.id
    for update skip locked
    limit p_limit
  loop
    v_min:=greatest(2,coalesce(nullif(v_comp.rules->>'min_participants','')::integer,2));
    select count(*) into v_registered
    from public.competition_registration r
    where r.competition_id=v_comp.id and r.state in ('REGISTERED','APPROVED');

    if v_registered<v_min then
      continue;
    end if;

    v_interval:=coalesce(nullif(v_comp.rules->>'round_interval_days','')::integer,7);
    if v_interval<1 or v_interval>90 then v_interval:=7; end if;

    begin
      perform public.service_activate_competition(v_comp.id,null,now(),v_interval);
      v_started:=v_started+1;
    exception when others then
      raise warning 'automatic competition activation failed for %: %',v_comp.id,sqlerrm;
    end;
  end loop;
...
$$;
revoke all on function public.service_auto_start_competitions(integer) from public,anon,authenticated;
grant execute on function public.service_auto_start_competitions(integer) to service_role;

-- Existing due SCHEDULED matches become playable immediately when this migration lands.
select public.service_ready_scheduled_matches(5000);
